...
const processCompletedCheckout = async (
  ...
  console.log('💳 Processing completed Stripe checkout:', session.id);
...
export const webhookCheckout = async (req: any, res: any) => {
  ...
  // ====================================================
  // PROCESS CHECKOUT AFTER ACKNOWLEDGING STRIPE
  // ====================================================

  if (event.type === 'checkout.session.completed') {
    const session = event.data.object as Stripe.Checkout.Session;

    void processCompletedCheckout(session).catch((error) => {
      console.error('❌ Background Stripe checkout processing failed:');
